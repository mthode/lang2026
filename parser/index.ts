import { scan, splitLogicalLines, type Token } from "../scanner/index.js";
...
export interface GenericParser {
  parseLine(line: string): StatementNode;
...
function parsePrimary(state: ParserState): ExpressionNode {
  ...
  if (token.type === "identifier") {
    ...
    while (peek(state)?.value === "(") {
      ...
      if (peek(state)?.value !== ")") {
        throw new Error("Expected ')' in call expression");
      }
      consume(state);
...
function parseArgumentValue(tokens: Token[], config: ParserConfig, commandDefinition: Required<CommandDefinition>): ArgumentValue {
  ...
  if (
    ...
    return parseExpressionFromTokens(compact.slice(1), config);
  }

  return parseExpressionFromTokens(compact, config);
}
...
function parseCommandArgumentsByDefinition(
  ...
  for (let index = 0; index < definitions.length; index += 1) {
    ...
    if (definition.vararg) {
      ...
      if (definition.kind === "nested-block") {
        while (varargCursor < tokens.length) {
          let parsed;
          try {
            parsed = parseValueByKind(tokens, varargCursor, definition, [], config, commandDefinition);
          } catch (error) {
    ...
    let parsed;
    try {
      parsed = parseValueByKind(tokens, cursor, definition, remainingNamedStops, config, commandDefinition);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      throw new Error(`Invalid value for argument '${definition.name}': ${message}`);
    }
...
function parseExpressionFromTokens(tokens: Token[], config: ParserConfig): ExpressionNode {
  const compact = compactTokens(tokens);
  if (compact.length === 0) {
    throw new Error("Expected expression");
  }

  const state: ParserState = { tokens: compact, index: 0, config };
  const value = parseExpression(state);

  if (state.index < compact.length) {
    const next = compact[state.index];
    throw new Error(`Unexpected token '${next?.value ?? ""}' after expression`);
  }
...
export function createParser(config: ParserConfig): GenericParser {
  function parseLine(line: string): StatementNode {
    const tokens = scan(line);

    const commandIndex = tokens.findIndex((token) => !isIgnorable(token));
    if (commandIndex < 0) {
      throw new Error("A command must start with an identifier");
    }
    ...
    if (config.allowAssignmentStatements && compactRemainder[0]?.type === "operator" && compactRemainder[0].value === "=") {
      const value = parseExpressionFromTokens(compactRemainder.slice(1), config);
      return {
        kind: "assignment",
        name,
        value,
    ...
    const args = commandDefinition.arguments.length > 0
      ? parseCommandArgumentsByDefinition(remainder, config, commandDefinition)
      : commandDefinition.consumeRestAsSingleArgument
        ? remainder.length > 0
          ? { arg0: parseArgumentValue(remainder, config, commandDefinition) }
          : {}
        : Object.fromEntries(
          splitArguments(remainder).map((tokensForArg, index) => [
            `arg${index}`,
            parseArgumentValue(tokensForArg, config, commandDefinition)
          ])
        );
  ...
  function parseScript(input: string): StatementNode[] {
    return splitLogicalLines(input).map((line) => parseLine(line));
  }
